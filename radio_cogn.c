...
float calculos(){
  ...
  float xfitness = fitness(sinr_pu, evaluacion_pu, aptitud_pu, pu_total, sinr_su, evaluacion_su, aptitud_su, su_total);
  printf("\nfitness: %f\n", xfitness);
  
  int global = 0;
  float max_taza_datos = 0.0, fitness = 0.0;
  // marcas(nombre, interacciones, global, estados_su, max_taza_datos, estado_del_espectro, sinr_pu, pu_total, sinr_su, su_total, su_grupo, fitness);
  return xfitness;
}

void main(){ 
  float result = calculos();
  printf("Global: %f\n", result);
}
